//
// Created by falcon on 2019/12/17.
//
...
int main(int argc, char *argv[]) {
    ...
    if (argc > 1) {
        ...
        frame = cv::imread("../bin/" + filename);
    } else {
        frame = cv::imread("../bin/cat.jpg");
    }
    // 分類器の読み込み
    string cascadeName = "../bin/cascade_cat.xml"; // Haar-like
    ...
    double scale = 1.0;        // 処理時間短縮のために画像縮小率]
    int facerect = 1;
    int key = -1;
