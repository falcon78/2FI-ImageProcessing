...
//	scale: 等方性スケーリング係数
//	出力: 2×3 の出力行列へのポインタ
    cv::Mat mapMatrix = cv::getRotationMatrix2D(center, angle, 1.0);
...
//	M: 2行 x 3列 の変換行列
//	dsize: 出力画像のサイズ
//	flags: 補間手法 (default CV_INNER_LINEAR (線形補間))
//	borderMode: 対応のとれない点を埋める方法 (default 0)
//  borderValue: 対応のとれない点を埋める値(CV_BORDER_CONSTANTの場合)
    cv::Mat result;
...
{
    char* args[4] = {argv[0],"../bin/sample.jpg","0"};
