//
// Created by falcon on 2019/10/29.
//
    ...
    switch (event) {
        case cv::MouseEventTypes::EVENT_LBUTTONDOWN: //左ボタンが押されたら
            ...
            if (!roiflag) {
                ...
                startp.x = x;
                startp.y = y;
                cv::circle(cImage, cv::Point(startp.x, startp.y), 3, cv::Scalar(0, 0, 255), 2, cv::LineTypes::LINE_AA);
                //ウィンドウ「roi」を閉じる
                ...
                endp.x = x;
                endp.y = y;
                cv::rectangle(cImage, cv::Point(startp.x, startp.y), cv::Point(endp.x, endp.y), cv::Scalar(0, 0, 255),
                              2, cv::LineTypes::LINE_AA);
                cv::imshow("input", cImage);
                //ROIの生成
...
void mouseEvent(char *fileName) {
    ...
    cv::setMouseCallback("input", onMouse, (void *) &image); //画像をコールバック関数に渡す
...
int main(int argc, char *argv[]) {
    if (argc < 2)
        mouseEvent("../bin/sample.jpg"); //default画像
    else
        mouseEvent(argv[1]);
